...
object LegacyService {
  ...
  object DataFetcher {
    def fetchAll(success: Boolean): Future[FetchResult] = {
  ...
  def program(): ZIO[Any, Object, Unit] = for {
    _ <- ZIO.logInfo("scheduled-job-start: genie-plus-sdp-scheduler")
    data <- ZIO.fromFuture {_: ExecutionContext => DataFetcher.fetchAll(success = true) }
    ...
    publishResult <- ZIO.fromFuture {_: ExecutionContext =>
      DataFetcher.produce(
        marshalledData._1,
        marshalledData._2,
        marshalledData._3,
